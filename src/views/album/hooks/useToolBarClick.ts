...
export function useToolBarClick() {
    ...
    const handleCancelSelect = () => {
        albumStore.clearSelectedFileList();
    };
    ...
    const handleIntoRecycle = async () => {
        ElMessageBox.confirm("删除的文件可在10天内找回", "删除文件", {
            ...
            const idList: number[] = [];
            albumStore.selectedFileList.forEach((item) => {
    ...
    const handleDownload = async () => {
        const selectList = albumStore.selectedFileList;
        ElMessageBox.confirm(`您将要下载 ${selectList.length} 个文件`, "下载文件", {
            ...
            // 依次请求
            for (const img of selectList) {
                const file = albumStore.preImgList.find((item) => item.fileId === img.fileId);
                // 构造a标签模拟点击
                ...
                ElMessage({
                    type: "success",
                    message: `${img?.fileName}.${img?.fileExt} 开始下载`,
                });
                setTimeout(() => {
                    indexStore.reloadApp();
                }, 300);
